...
namespace ariel {
    ...
    std::vector<std::unique_ptr<DevelopmentCard>>& Player::getDevelopmentCards() {
    ...
    int Player::myPoints(){
        ...
        int pointsFromVictoryCards = std::count_if(developmentCards.begin(), developmentCards.end(), [](const std::unique_ptr<DevelopmentCard>& card) { return dynamic_cast<VictoryPointCard*>(card.get()) != nullptr;});
        int pointsFromKnightCards= std::count_if(developmentCards.begin(), developmentCards.end(), [](const std::unique_ptr<DevelopmentCard>& card) { return dynamic_cast<KnightCard*>(card.get()) != nullptr;});
        // if the number of the knight cards are 3 then the player get 2 points.
        if (pointsFromKnightCards == 3) pointsFromKnightCards = 2;
